...
export default function MostLikelyPage() {
  ...
  useEffect(() => {
    if (room?.status === 'ml_finished' && room.status !== lastStatusRef.current && celebrationSound.current) {
      lastStatusRef.current = room.status
      celebrationSound.current.currentTime = 0
      celebrationSound.current.play().catch(() => {})
    } else if (room?.status !== 'ml_finished') {
    ...
    async function init() {
      ...
      channelRef.current = supabase.channel(channelName)
        ...
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'guesses' }, async (payload) => {
          ...
          // Only update if for our current game state
          setGuesses(prev => {
  ...
  // Vote count per player
  const voteCounts: Record<string, number> = {}
  for (const g of guesses) {
    voteCounts[g.guess] = (voteCounts[g.guess] ?? 0) + 1
  }

  // Winners: players with max votes
  const maxVotes = guesses.length > 0 ? Math.max(...Object.values(voteCounts), 0) : 0
  const winnerIds = maxVotes > 0 ? Object.entries(voteCounts).filter(([, c]) => c === maxVotes).map(([id]) => id) : []
      ...
      <div className="game-inner" style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div className="fade-up" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          ...
          {gameState?.total_rounds && (
            <span style={{ color: '#fff', fontWeight: 800, fontSize: 14, background: 'rgba(255,255,255,0.1)', padding: '6px 12px', borderRadius: 20 }}>
              {(gameState.current_question_idx ?? 0) + 1} / {gameState.total_rounds}
            </span>
          )}
        </div>
        ...
        {room?.status === 'ml_finished' ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: 24, textAlign: 'center' }}>
            ...
            <p style={{ color: T.muted, fontSize: 18, maxWidth: 400 }}>On sait maintenant qui est qui dans ce groupe !</p>
            {isHost ? (
              <Btn onClick={resetRoom} disabled={actioning} style={{ marginTop: 24, padding: '16px 32px', fontSize: 18, background: `linear-gradient(135deg, ${ORANGE} 0%, ${PINK} 100%)` }}>
                ...
                >
                  ...
                  <div style={{ textAlign: 'center' }}>
                    {isHost ? (
                      ...
                      <p style={{ color: T.muted, fontSize: 14 }}>Le host va passer à la question suivante...</p>
                    )}
                  </div>
                </motion.div>
              )}
